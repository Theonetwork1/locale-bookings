  ...
  <div className="min-h-screen bg-white p-6 flex flex-col justify-center">
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        <button onClick={() => navigate(Screen.DASHBOARD)} className="flex-1 py-4 bg-orange-500 text-white rounded-2xl font-bold shadow-lg hover:bg-orange-600 transition-all">
          Se connecter
        </button>
        <button onClick={() => navigate(Screen.DASHBOARD)} className="p-4 bg-slate-100 text-slate-800 rounded-2xl flex items-center justify-center hover:bg-slate-200">
...
export const RegisterScreen: React.FC<{ navigate: (s: Screen) => void }> = ({ navigate }) => (
  <div className="min-h-screen bg-white p-6 flex flex-col justify-center">
    ...
    <div className="space-y-4">
      ...
      <button onClick={() => navigate(Screen.DASHBOARD)} className="w-full py-4 bg-orange-500 text-white rounded-2xl font-bold shadow-lg hover:bg-orange-600 transition-all">
        S'inscrire
      </button>
    </div>
    <p className="mt-8 text-center text-gray-500">
...
export const ForgotPasswordScreen: React.FC<{ navigate: (s: Screen) => void }> = ({ navigate }) => (
  <div className="min-h-screen bg-white p-6 flex flex-col justify-center">
    ...
    <button onClick={() => navigate(Screen.OTP)} className="w-full py-4 bg-orange-500 text-white rounded-2xl font-bold">Envoyer le code</button>
    <button onClick={() => navigate(Screen.LOGIN)} className="mt-4 text-orange-500 font-bold text-center">Retour à la connexion</button>
  </div>
);
...
export const OTPScreen: React.FC<{ navigate: (s: Screen) => void }> = ({ navigate }) => (
  <div className="min-h-screen bg-white p-6 flex flex-col justify-center">
    ...
    <button onClick={() => navigate(Screen.DASHBOARD)} className="w-full py-4 bg-orange-500 text-white rounded-2xl font-bold">Vérifier</button>
  </div>
);
